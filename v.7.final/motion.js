(() => {
  ...
  if (hero.getBoundingClientRect().bottom > 0) {
    [...hero.querySelectorAll('.hero-kicker,h1,.hero-subtitle,.hero-description,.hero-actions')].forEach((el,i) => {
      play(el,[{opacity:0,transform:'translateY(12px)'},{opacity:1,transform:'translateY(0)'}],{duration:400,delay:i*80});
    });
    play(portrait,[{opacity:0,transform:'translateY(12px)'},{opacity:1,transform:'translateY(0)'}],{duration:400,delay:160});
  }
  ...
  function update() {
    ...
    cards.forEach((card,i)=>{
      ...
      const p=next?clamp((height-next.top)/(height-parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--header-height'))-88)):0;
